...
class RemindersLocalRepositoryTest {
    ...
    fun saveReminderAndRetrievesReminderById() = runBlocking {
        ...
        Assert.assertThat(res.data.title, `is`(item1.title))
        Assert.assertThat(res.data.description, `is`(item1.description))
        Assert.assertThat(res.data.location, `is`(item1.location))
        Assert.assertThat(res.data.latitude, `is`(item1.latitude))
        Assert.assertThat(res.data.longitude, `is`(item1.longitude))
        Assert.assertThat(res.data.id, `is`(item1.id))
    }
    ...
    fun saveRemindersAndRetrievesAllReminders() = runBlocking {
        ...
        res as Result.Success
        Assert.assertThat(res.data.size, `is`(3))
    }
    ...
    fun saveRemindersAndDeletesOneReminderById() = runBlocking {
        ...
        res as Result.Success
        Assert.assertThat(res.data.size, `is`(2))
        Assert.assertThat(res.data[0].location, `is`(item2.location))
    }
    ...
    fun saveRemindersAndDeletesAllReminders() = runBlocking {
        ...
        res as Result.Success
        Assert.assertThat(res.data.size, `is`(0))
    }
    ...
    fun getReminderAndReturnsError() = runBlocking {
        ...
        Assert.assertThat(res.message, `is`("Reminder not found!"))
    }
